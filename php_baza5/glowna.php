...
if(!$conn) {
    die("Wystąpił problem z połączeniem do serwera baz danych: ". mysqli_connect_error());
}
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["mainFormSubmit"])) {
    ...
    if(empty($errors)) {
        ...
        mysqli_stmt_bind_param($stmt, "ssissss", $imie, $nazwisko, $wiek_int, $plec, $miasto, $hobby, $wyksztalcenie);

        if(!mysqli_stmt_execute($stmt)){
            die("Wystąpił problem z wykonaniem zapytania!");
        }
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["usunFormSubmit"])) {
    ...
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssissss", $imieUsun, $nazwiskoUsun, $wiekUsun_int, $plecUsun, $miastoUsun, $hobbyUsun, $wyksztalcenieUsun);
...
if(!$result) {
    die("Wystąpił problem przy wykonywaniu zapytania!");
}
...
<body>
    <div id="main">
        ...
        <nav>
            <a href="glowna_pdo.php">glowna_pdo</a>
        </nav>
